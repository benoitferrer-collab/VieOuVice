...
export function AIWorkshop({
  ...
  userId: string;
  rpc: HubRpc;
  onChoose: (draft: CompetitionDraft) => void;
}) {
  ...
  useEffect(() => {
    ...
    rpc<Batch[]>("admin_ai_history")
      ...
      .catch(() => {
        if (alive.current)
          setError(
            "Exécute la mise à jour Atelier IA dans Supabase pour l’activer.",
          );
      });
    return () => {
      alive.current = false;
  ...
  async function generate() {
    ...
    try {
      ...
      if (!token || data.session?.user.id !== userId)
        throw Error("Reconnecte-toi avant de générer.");
      ...
      setBatches((old) =>
        [result, ...old.filter((b) => b.id !== result.id)].slice(0, 20),
      );
      intent.current = null;
    } catch (e) {
      if (alive.current)
        setError(e instanceof Error ? e.message : "Génération indisponible.");
    } finally {
  ...
  async function remove(
    ...
    setError("");
    try {
      const rows = await rpc<Batch[]>("admin_delete_ai_proposal", {
        p_batch_id: batchId,
        p_index: index,
        p_confirmation: confirmation,
      });
      if (alive.current) setBatches(rows);
      return true;
    } catch (e) {
      if (alive.current)
        setError(
          e instanceof Error
            ? e.message
            : "Suppression indisponible. Vérifie la mise à jour des propositions IA.",
        );
      return false;
    } finally {
      if (alive.current) setBusy(false);
  ...
  return (
    <section className="ai-workshop" aria-label="Atelier IA">
      <h3>
        <Sparkles size={18} /> Atelier de défis
      </h3>
      <p className="muted">
        Trois idées et leurs badges. Relis et adapte chaque proposition avant
        publication.
      </p>
      <label>
        Ambiance
      ...
      >
        {busy ? "Préparation des propositions…" : "Générer 3 défis"}
      </button>
      <p className="fine-print">
        Dix lots maximum par jour pour tout le jeu. Les propositions
        enregistrées sont réutilisables sans appel IA.
      </p>
      {error && (
