...
export function validPushEndpoint(endpoint: unknown): endpoint is string {
  ...
  if (
    !/^https:\/\/(?:fcm\.googleapis\.com|(?:[a-z0-9-]+\.)*push\.services\.mozilla\.com|web\.push\.apple\.com)\/[^\s#\\]+$/.test(
      endpoint,
    )
  )
    return false;
...
export function pushClickPath(
  ...
  const peer =
    selected === "messages" &&
    friendId &&
    /^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/.test(friendId)
      ? `&friend=${friendId}`
      : "";
  const duel =
    selected === "nemesis" &&
    duelId &&
    /^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/.test(duelId)
      ? `&duel=${duelId}`
      : "";
  return `/?tab=${selected}${peer}${duel}`;
}
export function pushPayload(
  ...
  return {
    ...
    url: pushClickPath(tab, friendId, duelId),
    ...(deliveryId &&
