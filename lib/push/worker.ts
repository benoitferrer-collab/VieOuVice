...
type Delivery = { endpoint: string; p256dh: string; auth: string; target_tab: string };
export async function dispatchPush(config: NonNullable<ReturnType<typeof pushServerConfig>>) {
  const db = createClient(config.url, config.secretKey, { auth: { persistSession: false, autoRefreshToken: false }, global: { fetch: (input, init) => fetch(input, { ...init, signal: AbortSignal.timeout(5000) }) } });
  let sent = 0;
  let processed = 0;
  ...
  while (processed < 10 && Date.now() < deadline) {
    ...
    const { data: current, error: validationError } = await db.rpc("authorize_push_job", { p_job_id: job.id, p_lease_token: job.lease_token });
    if (validationError) throw new Error("Push authorization unavailable");
    const delivery = current as Delivery | null;
    let result: "sent" | "expired" | "retry" | "failed" | "cancelled" = "cancelled";
    if (delivery && validPushEndpoint(delivery.endpoint)) {
      try {
        await webpush.sendNotification({ endpoint: delivery.endpoint, keys: { p256dh: delivery.p256dh, auth: delivery.auth } }, JSON.stringify(pushPayload(delivery.target_tab)), {
          TTL: 300,
          urgency: "normal",
          timeout: 5000,
    ...
    const { error: finishError } = await db.rpc("finish_push_job", { p_job_id: job.id, p_lease_token: job.lease_token, p_result: result });
