...
export const messageReactions = [
  ...
  { id: "strength", emoji: "💪", label: "Courage" },
  { id: "laugh", emoji: "😂", label: "Drôle" },
  { id: "heart", emoji: "❤️", label: "J’aime" },
] as const;
export type MessageReaction = (typeof messageReactions)[number]["id"];
export type MessageDraft = { body: string; key: string; emojiId?: string };
export const demoEmojiCatalog: CatalogueEmoji[] = [
  {
    id: "demo-star",
    label: "joie",
...
export type Message = {
  ...
  emoji_snapshot?: EmojiRecipe | null;
  created_at: string;
  read_at: string | null;
...
export function demoMessageRpc<T = unknown>(
  ...
  if (name === "get_emoji_catalog") {
    data = demoEmojiCatalog;
  } else if (name === "get_message_inbox") {
    ...
    for (const conversation of conversations as MessageInbox["conversations"]) {
      const last = mergeMessages(
        [],
        next.messages.filter(
          (m) =>
            (m.sender_id === user &&
              m.recipient_id === conversation.friend_id) ||
            (m.recipient_id === user && m.sender_id === conversation.friend_id),
        ),
      ).at(-1);
      conversation.last_message = last
        ? {
            body: [...last.body].slice(0, 120).join(""),
    ...
    conversations.sort(
      (a, b) =>
        ...
        a.friend_id.localeCompare(b.friend_id),
    );
    data = {
  ...
  } else if (name === "send_friend_message" || name === "send_friend_emoji") {
    const emoji =
      name === "send_friend_emoji"
        ? demoEmojiCatalog.find((e) => e.id === p.p_emoji)
        : undefined;
    if (name === "send_friend_emoji" && !emoji)
      throw Error("Emoji indisponible.");
    const body = emoji
      ? `Emoji : ${emoji.label}`
      : messageBody(String(p.p_body ?? ""));
    ...
    if (previous) {
      if (
        previous.recipient_id !== friend ||
        previous.body !== body ||
        previous.emoji_id !== emoji?.id
      )
      ...
      const m: Message = {
        ...
        body,
        ...(emoji
          ? { emoji_id: emoji.id, emoji_snapshot: { ...emoji.recipe } }
          : {}),
    ...
    if (
      !message ||
      ![message.sender_id, message.recipient_id].includes(user) ||
      !friends.includes(
        message.sender_id === user ? message.recipient_id : message.sender_id,
      )
    )
      throw Error("Message indisponible.");
    if (
      p.p_reaction !== null &&
      !messageReactions.some((r) => r.id === p.p_reaction)
    )
      throw Error("Réaction invalide.");
    const reactions = (message.reactions ?? []).filter(
      (r) => r.user_id !== user,
    );
    if (p.p_reaction !== null)
      reactions.push({
        user_id: user,
        reaction: p.p_reaction as MessageReaction,
      });
    data = { ...message, reactions };
    next.messages = next.messages.map((m) =>
      m.id === message.id ? (data as Message) : m,
    );
