...
function safePushPath(value) {
  try {
    ...
    if (
      ...
      url.pathname === "/" &&
      ["survie", "ligue", "nemesis", "amis", "messages"].includes(tab)
    ) {
      const friend = url.searchParams.get("friend");
      const suffix =
        tab === "messages" &&
        friend &&
        /^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/.test(friend)
          ? "&friend=" + friend
          : "";
      ...
      const arena =
        ...
        /^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/.test(duel)
          ? "&duel=" + duel
          : "";
      return "/?tab=" + tab + suffix + arena;
    }
  } catch {
    /* Use the application home for malformed payloads. */
  }
  return "/?tab=survie";
}
self.addEventListener("push", (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification("Du nouveau dans ta partie", {
      body: "Ouvre le jeu pour voir tes nouveautés.",
      tag:
        typeof data.deliveryId === "string" &&
        /^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/.test(data.deliveryId)
          ? "viegame-" + data.deliveryId
          : "viegame-social",
