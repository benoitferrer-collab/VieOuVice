...
test("dispatch fails closed and checks exact bearer token", () => {
  ...
  assert.equal(
    authorizedDispatch("Bearer " + "s".repeat(32), undefined),
    false,
  );
  assert.equal(
    authorizedDispatch("Bearer " + "s".repeat(32), "s".repeat(32)),
    true,
  );
  assert.equal(
    authorizedDispatch("bearer " + "s".repeat(32), "s".repeat(32)),
  ...
  assert.equal(
    authorizedDispatch("Bearer " + "x".repeat(32), "s".repeat(32)),
    false,
  );
  assert.equal(authorizedDispatch("Bearer short", "short"), false);
});
test("payload never includes identity, action or event text", () => {
  assert.deepEqual(pushPayload("amis"), {
    title: "Du nouveau dans ta partie",
    url: "/?tab=amis",
  });
  assert.deepEqual(pushPayload("https://evil.test"), {
    title: "Du nouveau dans ta partie",
    url: "/?tab=survie",
  });
...
test("conversation links preserve only valid peer UUIDs on the Messages tab", () => {
  ...
  assert.equal(
    pushClickPath("messages", peer),
    `/?tab=messages&friend=${peer}`,
  );
  assert.equal(pushClickPath("amis", peer), "/?tab=amis");
  assert.equal(
    pushClickPath("messages", "https://evil.test"),
    "/?tab=messages",
  );
});

test("arena links retain a valid duel only in the Némésis tab", () => {
  const duel = "12345678-1234-4123-8123-123456789abc";
  assert.equal(
    pushClickPath("nemesis", undefined, duel),
    `/?tab=nemesis&duel=${duel}`,
  );
  assert.equal(pushClickPath("messages", undefined, duel), "/?tab=messages");
  assert.equal(
    pushClickPath("nemesis", undefined, "https://evil.test"),
    "/?tab=nemesis",
  );
  assert.equal(
    pushPayload("nemesis", undefined, undefined, duel).url,
    `/?tab=nemesis&duel=${duel}`,
  );
});
