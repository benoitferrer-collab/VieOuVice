...
test("service worker ignores untrusted titles and constrains every click to game tabs", async () => {
  ...
  assert.equal(
    listeners.has("fetch"),
    false,
    "No private responses are cached",
  );
  for (const url of [
    "https://evil.example/?tab=amis",
    "//evil.example/?tab=amis",
    "javascript:alert(1)",
    "/private?tab=amis",
    "/?tab=not-a-tab",
  ]) {
  ...
  listeners.get("notificationclick")!({
    notification: { close() {}, data: { url: "/?tab=nemesis&secret=ignored" } },
    waitUntil: (p: Promise<unknown>) => {
      pending = p;
    },
  });
  ...
  runInNewContext(
    readFileSync(new URL("../public/sw.js", import.meta.url), "utf8"),
    {
      ...
      self: {
        ...
        registration: {
          showNotification: async (_: string, options: { tag: string }) => {
...
test("service worker preserves the conversation link but strips unrelated query data", async () => {
  ...
  for (const [url, expected] of [
    [
      `/?tab=messages&friend=${peer}&body=private`,
      `/?tab=messages&friend=${peer}`,
    ],
    ["/?tab=messages&friend=invalid", "/?tab=messages"],
    [`/?tab=nemesis&duel=${peer}&body=private`, `/?tab=nemesis&duel=${peer}`],
    ["/?tab=nemesis&duel=invalid", "/?tab=nemesis"],
    [`/?tab=amis&duel=${peer}`, "/?tab=amis"],
  ]) {
    let pending: Promise<unknown> = Promise.resolve();
